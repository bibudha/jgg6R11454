...
function Merchandisecategory() {
    ...
    doAjaxCall(url, data, false, function (html) {
        if ($.isEmptyObject(html)) {
			...
			//alert("coming");
			  $.each(html, function (i, item) {
			  data += '<li><a rel="external" href="merchandise_magento_item.html?location=' + location + '&skuId=' + item.sku + '&productId=' + item.product_id + '&touchId=' + userSiteId + '" data-transition="slide">' + item.name + '</a></li>';
			...
			var str ="";
				...
				/*$.each(html,function (i,item){
				str += '<div data-role="collapsible-set" data-theme="a" data-content-theme="a" id="col_div">';
					$.each(item.children,function(j,child){
						str +='<div data-role="collapsible" id="col_div1">';
    					str  += '<h3>'+child.name+'</h3>';
						str += '<ul data-role="listview" id="aboutclass">';
						str += '<li><a href="#">Canary</a></li>';
						str +='</div>';	
						alert(child.name);
				...
				try {
					//$( "#col_div" ).collapsibleset( "refresh" );
					$( "#col_div1" ).collapsibleset( "refresh" );
                    $("#aboutclass").listview('refresh');
                } catch (e) {
                    $("#aboutclass").listview();
                }*/
...
//alert('coming');
		if ($.isEmptyObject(html)) {
			...
			$("#item_Img").val(item.imgpath);
			$("#itemImg").attr("src",item.imgpath);
			$("#item_Desc").val(itemDesc);
			$("#merchId").val(item.merchId);
			$("#con_fee").val(item.con_fee);
			});
...
var location=$("#location").val();
 ...
 var url = baseUrl + 'web/web/insertMagentoCart/'+sess_id;
    ...
    doAjaxCall(url, data, true, function (html) {
        if (html != 0) {
            //console.log(data);
			window.location.href = 'magento_checkout_cart.html?touchId=' + touchId + '&location=' + location + '&last_id=' + html;
        } else {
...
function merchandisecheckoutcart() {
    ...
    doAjaxCall(url, '', false, function (validationData) {
        ...
        if ($.isEmptyObject(validationData)) {
            ...
            url2 = baseUrl + 'web/web/merchgetcartInfo/' + sess_id + '/' + featureRelId + '/' + userSiteId + '/' + location;
...
var userSiteId = getUrlVars()['touchId'];
var location = getUrlVars()['location'];
var last_id = getUrlVars()['last_id'];

var data='';
var url = baseUrl + 'web/web/magentocartinfo/' + userSiteId + '/' + location + '/' + last_id ;
...
function submitmerchandiseForm() {
    ...
    var url = baseUrl + 'web/web/doDirectMerchandPayment/'+ merch_id + '/' + sess_id;
